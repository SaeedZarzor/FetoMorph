...
Example config: `configs/crop_band_config.example.json`

Use the Python 3.12 `.venv` described in `AREA_BAND_CLI_RUN.md`. For a fresh
checkout, create the local crop config before editing it:

```powershell
Copy-Item configs\crop_band_config.example.json configs\crop_band_config.json
```
...
Configuration values can also be overridden on the command line. Run
`python -m functions.crop_band_roi --help` for the complete option list,
including input root, axes, bounding box, margins, validation, output naming,
and scale-bar controls.

...
FetoMorph extracts quantitative morphometric descriptors of the developing fetal brain — surface area, perimeter, volume, gyrification (LGI / GI), compactness, curvature, and a full sulcal profile (per-class counts and depths, primary / secondary / tertiary) — from real imaging data across 2D sections, 3D volumes (NIfTI), and surface meshes (STL/VTK). These descriptors are indexed by gestational age and summarised into reference growth profiles.

**Aim.** The core purpose of the project is to **validate the simulation results generated by computational models of brain development**. Computational models of cortical folding and brain growth produce simulated geometries at successive developmental time points; FetoMorph measures those simulated outputs with exactly the same pipeline used on real fetal brain data, and then compares them against age-specific reference statistics. The **Gestational Age Similarity Profile (GASP)** scores how closely a measured (real or simulated) brain matches each gestational week, giving an objective, metric-based way to judge whether a simulation reproduces realistic developmental morphology and timing.

In short, FetoMorph turns "does this simulated brain look developmentally plausible?" into a reproducible set of measurements and a similarity score against real-data references.

> **Authorship.** The underlying methodology, concept, and scientific investigation were developed by **Saeed Zarzor**. The software implementation was written with the assistance of **Claude Code** and **Codex**.

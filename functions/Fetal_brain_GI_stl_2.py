...
def compute_stl_lGI(file_path: str, out_dir: str, cnt_threshold: float=20, kernel_size: int=5, slice_thickness: float=0.5,):

    # Load mesh
    mesh = pv.read(str(file_path))

    print(f"[STL lGI] mesh data is {mesh}")

    # Brain real dimensions in mm
    x_min, x_max, y_min, y_max, z_min, z_max = mesh.bounds
    brian_dim = [x_max - x_min, y_max - y_min, z_max - z_min]
    print(f"[STL lGI] Brain dimensions (mm): {brian_dim}")
    ...
    if N == 0:
        print("[STL lGI] No slices to process.")
        return
...
#    pv.global_theme.off_screen = True  # ensure all plotters default to off-screen
    ...
    if N  > 1:
        for idx, y in enumerate(slice_positions):
            ...
            scale_cube = pv.Cube(x_length=cube_length, y_length=1, z_length=cube_length)
            ...
            image = p.screenshot(return_img=True, filename=f"{out_dir}/image_{idx:03d}.png")
    ...
    #        cv2.putText(imag, f"Perimeter:{inner_perimeter:.2f}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, font_scale, (255, 0, 200), 2)
            ...
            cv2.imwrite(slice_path, imag)
            valid_slices.append(idx)
            saved_pngs.append(slice_path)

            sheet.append([idx, inner_perimeter, outer_perimeter , GI_slice])
        ...
        print(f"[STL lGI] GI_total equls to {GI_total}")

        sheet.append(["GI", round(GI_total, 3)])

        # Save to Excel
        # Save to Excel
        df = pd.DataFrame(sheet, columns=["Slice", "Inner_Perimeter ", "Outer_Perimeter ", "GI"])
        ...
        if solids:
            ...
            solid_volume.save(solid_path)

        return GI_total, saved_pngs, valid_slices

    else:
        lGI, saved_pngs, valid_slices = 0, [], []
        
if __name__ == "__main__":
    
    file_path = "/Users/saeed/Library/Mobile Documents/com~apple~CloudDocs/Documents/FetoMorph/Data/fetal_surface/24week_3day_sub-CC00862XX13_ses-41210/brain_both.stl"
    out_dir = "/Users/saeed/Library/Mobile Documents/com~apple~CloudDocs/Documents/FetoMorph/folder"
    lgi,saved_pngs,valid_slices = compute_stl_lGI(file_path, out_dir, 20, 5, 0.5)
    print(lgi)
